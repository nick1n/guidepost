<script lang="ts">
  import type { Edition } from "$lib/kdm-data";
  ...
  let { editions, value, copyNumbers, onselect, onSetCopyNumber }: Props =
    $props();

  const selected = $derived(
    editions.filter((edition) => value.includes(edition.v)),
  );
</script>
...
<div class="flex flex-col gap-2">
  <div
    ...
    role="group"
    aria-label="Beta editions owned"
  >
      ...
      <input
        ...
        oninput={(event) => {
          ...
          onSetCopyNumber(
            ...
            raw === ""
              ? undefined
              : Math.min(999, Math.max(1, Number.parseInt(raw, 10) || 1)),
          );
        }}
